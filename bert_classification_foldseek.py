...
maxlen = 256
batch_size = 64
config_path = 'tk/config_foldseek.json'
checkpoint_path = None
dict_path = 'tk/vocab_foldseek.txt'
        ...
        for filename in filenames:
            with open(filename, encoding='utf-8') as f:
                for l in f:
                    ...
                    #     print(t)
                    D.append((" ".join(text), label)) # 1-7 to 0-6
        return D
...
# 加载数据集
train_dataloader = DataLoader(MyDataset(['tk/EC/EC_foldseek_train.txt']), batch_size=batch_size, shuffle=True, collate_fn=collate_fn) 
valid_dataloader = DataLoader(MyDataset(['tk/EC/EC_foldseek_valid.txt']), batch_size=batch_size, collate_fn=collate_fn) 
test_dataloader = DataLoader(MyDataset(['tk/EC/EC_foldseek_test.txt']),  batch_size=batch_size, collate_fn=collate_fn) 
...
class Evaluator(Callback):
    ...
    def evaluate(self, data):
        ...
        for x_true, y_true in data:
            ...
            # 存储预测结果和真实标签，这里假设预测结果和真实标签都是张量
            preds.append(y_pred.detach())
            target.append(y_true)
        
        # 使用all_gather聚合所有进程的预测结果和目标标签
